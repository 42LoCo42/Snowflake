...
import Data.List (group, inits, sort)
import Primitives
import SaveState
import Snowflake
...
pairsquares [_] = []
pairsquares program =
  filter (\(e, _) -> canChoose e)
    $ map (\l@(e : _) -> (e, length l ^ (2 :: Integer)))
    $ group
    $ sort
...
chooseLength (a, b) dep (Just old_dep) program full_program =
  if old_dep == a || old_dep == b
    then return $ highest + 1
    else do
